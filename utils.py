...
logging.basicConfig(
    ...
    level=logging.INFO, 
    format="%(asctime)s - %(levelname)s - %(message)s", # 这里可以自定义加入开头信息
    # 添加线程信息
    # format="%(asctime)s - %(levelname)s - [Thread-%(thread)d] (%(threadName)s) - %(message)s",
    encoding='utf-8' 
)

client = OpenAI(api_key=os.getenv("DS_API_KEY"),base_url=os.getenv("DS_API_BASE_URL"))
warnings.simplefilter("always")

GOOGLE_KEY   = '0d8265613c091abbfd920f52e8b4937dece72d0b'
arxiv_client = arxiv.Client(delay_seconds = 0.05)
arxiv._DEFAULT_USER_AGENT = "MyAgent/0.1 (1713476245@qq.com)"
# 每 1 秒最多 10 次请求
limiter = RateLimiter(max_calls=10, period=1)
id2paper     = json.load(open("/root/autodl-tmp/pasa-dataset/paper_database/id2paper.json"))
paper_db     = zipfile.ZipFile("/root/autodl-tmp/pasa-dataset/paper_database/cs_paper_2nd.zip", "r")
        ...
        if child_type == bs4.element.NavigableString:
            ...
            continue
                ...
                if child.name in ignore_tags or (child.has_attr('class') and child['class'][0] == 'navigation'):
                    ...
                    local_text.append(r'~\cite{' + ', '.join(hrefs) + '}')
...
def parse_html(html_file):
  ...
  # 或者使用：os.getenv("OPENAI_API_KEY")

def extract_title_with_llm(citation: str) -> str:
    """
    使用 GPT-4o 从引用字符串中提取论文标题。
    Args:
        citation (str): 引用的原始字符串（如 BibTeX 或普通引文格式）
    Returns:
        str: 提取出的论文标题，若失败则返回原始字符串
    """
    prompt = (
        "Extract the paper title from the following reference string.\n"
        "Only return the exact title without any author, journal or year info.\n\n"
        f"Reference: \"{citation}\"\n\n"
        "Title:"
    )

    try:
        response = client.chat.completions.create(model=os.getenv("DS_R1_MODEL_32B"),
        messages=[
        ...
        warnings.warn(f"LLM title extraction failed: {e}")
                ...
                for k, v in sections.items():
                    ...
                    for bib in bibs:
                        ...
                        for b in bib:
                            ...
                            raw_citation = document["references"][b]["meta_string"]
                            title = extract_title_with_llm(raw_citation)  # 调用你自己实现的函数
...
def search_arxiv_id_by_title(title):
    ...
    for result in arxiv_client.results(search):
        ...
        return arxiv_id
    logging.warning(f"Failed to search arxiv id by title: {title}")
    return None
...
#             if soup.find('meta', charset=True): # paper list <meta charset="utf-8">
